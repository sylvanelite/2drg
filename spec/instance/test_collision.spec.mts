...
describe("test collision",function(){
	it("test collision found x",async function() {
        ...
        //set up so that A overlaps B, and C overlaps B, but A and C don't overlap
        entityA.size.x = 8;
        entityA.size.y = 8;
        entityB.size.x = 8;
        entityB.size.y = 8;
        ...
        entityB.position.y = 6;//entB is offset by 2 but should still overlap
        ...
        let CollideB = 0;
        let CollideC = 0;
        Collision.checkCollisions(room,entityA,(collisionId:number)=>{
            const ent = room.entities[collisionId];
            expect(ent.uid).not.toBe(entityC.uid);
            expect(ent.uid).not.toBe(entityA.uid);
            CollideA+=1;
        });
        Collision.checkCollisions(room,entityB,(collisionId:number)=>{
            const ent = room.entities[collisionId];
            expect(ent.uid).not.toBe(entityB.uid);
            CollideB+=1;
        });
        Collision.checkCollisions(room,entityC,(collisionId:number)=>{
            const ent = room.entities[collisionId];
            expect(ent.uid).not.toBe(entityA.uid);
            expect(ent.uid).not.toBe(entityC.uid);
            CollideC+=1;
	...
	it("test collision found y",async function() {
        ...
        entityA.position.x = 0;
        entityB.position.x = 6;//entB is offset by 2 but should still overlap
	...
	it("test collision large objects",async function() {
        ...
        entityA.position.y = 0;
        entityB.position.y = 60;//entB is offset by 2 but should still overlap
        entityC.position.y = 0;
        Room.AddEntity(room,entityA);
        Room.AddEntity(room,entityB);
        Room.AddEntity(room,entityC);
